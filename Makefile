...
podman-build: build
	podman build --tag umamurl --build-arg TARGETARCH=amd64 -f Dockerfile.alpine .

podman-stop:
	podman ps -q --filter "name=umamurl" | xargs -r podman stop
	podman ps -aq --filter "name=umamurl" | xargs -r podman rm
...
podman-test: test podman-build podman-stop
	podman run -t -p ${port}:${port} --name umamurl --env-file ./.env -v "${db_dir}:/data" -d umamurl
	podman logs umamurl -f 

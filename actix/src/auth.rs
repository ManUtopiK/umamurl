    ...
    if config.api_key.is_some() {
        ...
        if get_api_header(&http).is_some() {
            JSONResponse {
                success: false,
                error: true,
                reason: "An API key was provided, but the 'api_key' environment variable is not configured in the Umamurl instance".to_string(), 
            }
        } else {
            JSONResponse {
                success: false,
...
pub fn is_key_valid(key: &str, config: &Config) -> bool {
    if let Some(api_key) = &config.api_key {
        ...
        if !authorized {
            warn!("Incorrect API key was provided when connecting to Umamurl.");
            false
        } else {
...
// Validate a session
pub fn is_session_valid(session: Session, config: &Config, db: &Connection) -> bool {
    // Check env var password first, then DB-stored password
    let has_password = config.password.is_some() || database::get_setting("password", db).is_some();

    // If no password is configured anywhere, allow access (setup mode)
    if !has_password {
        return true;
    }

    if let Ok(token) = session.get::<String>("umamurl-auth") {
...
// Check if any password is configured (env var or DB)
pub fn is_password_configured(config: &Config, db: &Connection) -> bool {
    config.password.is_some() || database::get_setting("password", db).is_some()
}

// Validate a password against the DB-stored hash (always Argon2)
pub fn validate_db_password(input: &str, db: &Connection) -> bool {
    if let Some(stored_hash) = database::get_setting("password", db) {
        let Ok(hash) = PasswordHash::new(&stored_hash) else {
            warn!("Invalid password hash stored in database.");
            return false;
        };
        Argon2::default()
    ...
    if let Some(token_body) = token {
        ...
        if token_parts.len() < 2 {
            ...
            let time_now = SystemTime::now()
                ...
                .as_secs();
            token_text == "umamurl-auth" && time_now < token_time + 1209600 // There are 1209600 seconds in 14 days
        }
    } else {
        false
    }
...
pub fn gen_token() -> String {
    let token_text = String::from("umamurl-auth");

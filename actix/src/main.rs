...
async fn main() -> Result<()> {
    env_logger::builder()
        .parse_filters(
            std::env::var("RUST_LOG")
                .ok()
                .filter(|s| !s.is_empty())
                .unwrap_or("warn,umamurl=info,actix_session::middleware=error".to_string())
                .as_str(),
        )
        .format(|buf, record| {
            use chrono::Local;
    ...
    eprintln!("----------------------------------------------------------------------");
    info!("Starting Umamurl Server v{}", env!("CARGO_PKG_VERSION"));
    info!("Source: https://github.com/ManUtopiK/umamurl");
    eprintln!("----------------------------------------------------------------------");
    ...
    HttpServer::new(move || {
        let mut app = App::new()
            ...
            .service(services::set_password)
            .service(services::expand)
            .service(services::whoami)
            .service(services::get_umami_config)
            .service(services::set_umami_config)

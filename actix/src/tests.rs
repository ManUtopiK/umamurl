...
fn default_config(test: &str) -> config::Config {
    ...
    port: 4567,
    db_location: format!("/tmp/umamurl-test-{test}.sqlite"),
    ...
    let app = test::init_service(
        App::new()
            .app_data(web::Data::new(AppState {
                db: database::open_db(
                    format!("/tmp/umamurl-test-{test}.sqlite").as_str(),
                    conf.use_wal_mode,
                    conf.ensure_acid,
                ),
                config: conf.clone(),
...
async fn edit_link<T: Service<Request, Response = ServiceResponse, Error = Error>>(
    ...
    let req = test::TestRequest::put()
        .uri("/api/edit")
        .insert_header(("X-API-Key", api_key))
        .set_payload(format!("{{\"shortlink\":\"{shortlink}\",\"longlink\":\"https://edited-{shortlink}.com\"}}"))
        .to_request();
    let resp = test::call_service(&app, req).await;
    resp.status()
}
...
async fn basic_site_config() {
    ...
    let site_url = conf.site_url.clone().unwrap();
    ...
    let resp = test::call_service(&app, req).await;
    let body = to_bytes(resp.into_body()).await.unwrap();
    assert_eq!(body.as_str(), site_url);
    ...
    assert_eq!(
        body.as_str(),
        format!("Umamurl v{}", env!("CARGO_PKG_VERSION"))
    );
...
async fn link_editing() {
    ...
    let status = edit_link(&app, &api_key, "test2").await;
    assert!(status.is_success());
    ...
    assert!(resp.status().is_redirection());
    let status = edit_link(&app, &api_key, "test1").await;
    assert!(status.is_success());

...
use crate::{
    ...
    database,
    services::{
        UmamurlError::{self, ClientError, ServerError},
        GetReqParams,
    },
};
...
) -> Result<(String, i64), UmamurlError> {
...
pub fn edit_link(req: &str, db: &Connection, config: &Config) -> Result<(), UmamurlError> {
    ...
    let result = database::edit_link(&chunks.shortlink, &chunks.longlink, db);
    match result {

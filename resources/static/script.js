...
const showVersion = () => {
  ...
  if (VERSION) {
    ...
    link.href =
      "https://github.com/ManUtopiK/umamurl/releases/tag/" + VERSION;
    link.hidden = false;
  } else {
    link.hidden = true;
  }
...
const showLogin = () => {
  document.getElementById("container").classList.add("container-blur");
...
const displayData = () => {
  ...
  admin_button.hidden = false;
  document.getElementById("settings-button").hidden = !ADMIN;
  updateInputBox();
...
const refreshExpiryTimes = async () => {
  const tds = document.getElementsByClassName("expiry-tooltip");
  for (let i = 0; i < tds.length; i++) {
    ...
    if (relativeTime == "expired") {
      ...
      const actionsCell = td.parentElement.querySelector(
        'td[name="actions"]',
      );
      if (actionsCell) {
        for (const btn of actionsCell.querySelectorAll("button")) {
          btn.disabled = true;
        }
      }
    }
...
const TD = (s, label, extraClasses) => {
  const td = document.createElement("td");
  td.className = "px-4 py-3 " + (extraClasses || "");
  ...
  div.innerHTML = s;
  td.appendChild(div);
  if (label) td.setAttribute("label", label);
  return td;
};
...
const TR = (i, row) => {
  ...
  const numTD = TD(i, null, "text-gray-400 w-10");
  numTD.setAttribute("name", "numColumn");

  const longlink = row["longlink"];
  const longTD = TD(A_LONG(longlink), "Long URL", "break-all");

  const shortlink = row["shortlink"];
  tr.id = shortlink;
  const shortTD = TD(A_SHORT(shortlink), "Short URL", "font-medium");
  ...
  if (expiryTime && expiryTime > 0) {
    ...
    expiryHTML =
      relativeExpiryTime +
      '<span class="expiry-tooltiptext">' +
      accurateExpiryTime +
      "</span>";
  }

  const expiryTD = TD(expiryHTML, "Expiry", "text-center text-gray-400");
  if (expiryTime && expiryTime > 0) {
    expiryTD.setAttribute("data-time", expiryTime);
    expiryTD.classList.add("expiry-tooltip");
  }
  ...
  btnGrp.className = "flex items-center justify-center gap-1";
  ...
  for (const td of [numTD, shortTD, longTD, expiryTD, actionsTD]) {
    tr.appendChild(td);
  }
  return tr;
};
...
const copyShortUrl = (shortLink, doCopy) => {
  ...
  const linkElt = `<a href=${fullLink} target="_blank" class="text-blue-400 underline">${fullLink}</a>`;
...
const A_LONG = (s) =>
  `<a href='${s}' target="_blank" class="text-blue-400 hover:underline">${s}</a>`;
const A_SHORT = (s) =>
  `<a href="${SITE_URL}/${s}" target="_blank" class="text-blue-400 hover:underline">${s}</a>`;

const svgButton = (svgContent, title) => {
...
const copyButton = (shortUrl) => {
  const btn = svgButton(SVG_COPY_BUTTON, "Copy Short URL");
  btn.onclick = (e) => {
    e.preventDefault();
...
const editButton = (shortUrl, longUrl) => {
  const btn = svgButton(SVG_EDIT_BUTTON, "Edit Short URL");
  btn.onclick = () => {
    document.getElementById("container").classList.add("container-blur");
...
const qrCodeButton = (shortlink) => {
  const btn = svgButton(SVG_QR_BUTTON, "Show QR Code");
  btn.onclick = () => {
    ...
    qrDown.download = `umamurl-qr-${shortlink}.png`;
    document.getElementById("container").classList.add("container-blur");
...
const deleteButton = (shortUrl) => {
  const btn = svgButton(SVG_DELETE_BUTTON, "Delete Short URL");
  btn.onclick = (e) => {
    e.preventDefault();
      ...
      "text/plain": fetch(url, payload)
        ...
        .then((shortUrl) => {
          if (ok) {
            ...
            showAlert(shortUrl, "rgb(153 27 27 / 0.2)");
          }
        })
        .catch((err) => {
          console.log("Error:", err);
...
const submitEdit = () => {
  ...
  if (confirm("Are you sure that you want to edit " + shortUrl + "?")) {
    ...
    fetch(url, {
      ...
      .then(async (text) => {
        if (!ok) {
          showAlert(text, "rgb(153 27 27 / 0.2)");
        } else {
...
const submitSetup = () => {
  ...
  fetch(prepSubdir("/api/set-password"), {
    method: "POST",
    cache: "no-cache",
    body: password,
  })
    .then(async (res) => {
      if (res.ok) {
        ...
        document.getElementById("container").classList.remove("container-blur");
        CONFIG.password_configured = true;
        // Now require login with the new password
        ADMIN = false;
        VERSION = null;
        LOCAL_DATA = [];
        await refreshData();
        ...
        const data = await res.json();
        errorEl.textContent = data.reason || "Failed to set password.";
        errorEl.hidden = false;
      }
    })
    .catch((err) => {
...
const openSettings = async () => {
  ...
  const envNotice = document.getElementById("settings-env-notice");
  const saveBtn = document.getElementById("settings-save-button");
  const umamiUrlInput = document.getElementById("umami-url");
  const umamiIdInput = document.getElementById("umami-website-id");

  // Reset to editable state by default
  envNotice.hidden = true;
  umamiUrlInput.disabled = false;
  umamiIdInput.disabled = false;
  saveBtn.hidden = false;

  try {
    const res = await fetch(prepSubdir("/api/umami-config"), {
      cache: "no-cache",
    });
    if (res.ok) {
      const data = await res.json();
      umamiUrlInput.value = data.umami_url || "";
      umamiIdInput.value = data.umami_website_id || "";
      if (data.env_configured) {
        envNotice.hidden = false;
        umamiUrlInput.disabled = true;
        umamiIdInput.disabled = true;
        saveBtn.hidden = true;
      }
    }
  } catch (err) {
...
const submitSettings = () => {
  const umamiUrl = document.getElementById("umami-url").value.trim();
  const websiteId = document.getElementById("umami-website-id").value.trim();
  const statusEl = document.getElementById("settings-status");

  fetch(prepSubdir("/api/umami-config"), {
    method: "POST",
    cache: "no-cache",
    ...
    .then(async (res) => {
      ...
      if (res.ok) {
        ...
        statusEl.textContent = data.reason || "Failed to save.";
...
refreshData()
  .then(() => {
    ...
    document.forms.namedItem("settings-form").onsubmit = (e) => {
      e.preventDefault();
      submitSettings();
    };
    ...
    document.forms.namedItem("setup-form").onsubmit = (e) => {
      e.preventDefault();
      submitSetup();
    };
    document.getElementById("setup-skip-button").onclick = () => {
      setupDialog.close();
    };
  })
  .catch((err) => {
